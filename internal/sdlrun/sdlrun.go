...
func RunWindow(e *engine.Engine, scale int) error {
    if scale <= 0 { scale = 2 }
    if err := sdl.Init(sdl.INIT_VIDEO); err != nil { return err }
    defer sdl.Quit()
    ...
    for running {
        ...
        if len(pix) > 0 { ptr = unsafe.Pointer(&pix[0]) }
        if err := tex.Update(nil, ptr, e.Ren.Width()*4); err != nil { return err }
        if err := ren.Clear(); err != nil { return err }
        if err := ren.Copy(tex, nil, nil); err != nil { return err }
        ren.Present()
    }
    return nil
}
