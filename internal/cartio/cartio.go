...
func Write(w io.Writer, m Manifest, assets []Asset) error {
    ...
    for _, a := range assets {
        ...
        if err != nil { return err }
        if _, err := f.Write(a.Data); err != nil { return err }
    }
    return zw.Close()
}
...
func Read(r io.ReaderAt, size int64) (Manifest, map[string][]byte, error) {
    ...
    for _, f := range zr.File {
        ...
        if _, err := io.Copy(&buf, rc); err != nil { rc.Close(); return Manifest{}, nil, err }
        rc.Close()
        if f.Name == "manifest.json" {
            if err := json.Unmarshal(buf.Bytes(), &m); err != nil { return Manifest{}, nil, err }
            continue
        }
