...
func packDir(dir, out string) error {
	...
	if err := json.Unmarshal(mfBytes, &m); err != nil { return err }
	...
	if err := cartio.Write(&buf, m, assets); err != nil { return err }
...
func main() {
	...
	if *cart != "" {
		e := engine.New(60)
		defer e.Close()
		if err := e.LoadCartFile(*cart); err != nil { panic(err) }
		if *window {
		...
		e.RunFrames(*frames)
		if *out != "" {
			if err := savePNG(*out, e.Ren.Width(), e.Ren.Height(), e.Ren.Pixels()); err != nil { panic(err) }
			println("wrote:", *out)
		} else {
			pix := e.Ren.Pixels()
